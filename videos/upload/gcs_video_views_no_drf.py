...
@csrf_exempt
@jwt_required
def gcs_initiate_video_upload(request):
    ...
    try:
        ...
        logger.exception("GCS Video Initiate Failed")
        return JsonResponse({"error": str(e)}, status=500)

@csrf_exempt
@jwt_required
def gcs_get_video_part_url(request):
    if request.method != 'POST':
        return JsonResponse({'error': 'Only POST allowed'}, status=405)
...
@csrf_exempt
@jwt_required
def gcs_complete_video_upload(request):
    if request.method != 'POST':
        return JsonResponse({'error': 'Only POST allowed'}, status=405)
