...
def gcs_initiate_track_upload(request):
    ...
    try:
        ...
        logger.exception("GCS Track Initiate Failed")
        return JsonResponse({"error": str(e)}, status=500)

@csrf_exempt
@jwt_required_testable
def gcs_get_track_part_url(request):
    if request.method != 'POST':
        return JsonResponse({'error': 'Only POST allowed'}, status=405)
...
@csrf_exempt
@jwt_required_testable
def gcs_complete_track_upload(request):
    if request.method != 'POST':
        return JsonResponse({'error': 'Only POST allowed'}, status=405)
